...
/// @author scx567888
/// @version 0.0.1
public final class ScxMediaTypeHelper {

    public static final Pattern SEMICOLON_PATTERN = Pattern.compile(";\\s*");

    public static ScxMediaTypeWritable parseMediaType(String str) throws IllegalArgumentException {
        var parts = SEMICOLON_PATTERN.split(str);

        //处理意外情况
        if (parts.length == 0) {
            throw new IllegalArgumentException("Invalid MediaType: " + str);
        }
        ...
        var m = new ScxMediaTypeImpl(split[0], split[1]);
