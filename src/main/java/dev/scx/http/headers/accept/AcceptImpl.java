...
import static dev.scx.http.headers.accept.AcceptHelper.sortMediaRanges;

/// AcceptsImpl
///
/// @author scx567888
/// @version 0.0.1
final class AcceptImpl implements AcceptWritable {
    ...
    public final <T extends ScxMediaType> T negotiate(T... serverSupportedTypes) {
        var sorted = sortMediaRanges(mediaRanges);
        // 遍历已排序的 MediaRange, 找到第一个匹配的服务端支持的类型
        for (MediaRange range : sorted) {
