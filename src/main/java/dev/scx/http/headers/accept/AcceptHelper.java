...
public final class AcceptHelper {
    ...
    public static AcceptWritable parseAccept(String s) throws IllegalMediaRangeException {
        var list = new ArrayList<MediaRange>();
        var mediaTypes = s.split(",");
        for (var mediaType : mediaTypes) {
            list.add(MediaRange.parse(mediaType.trim()));
        }
        return new AcceptImpl(list);
    }
    ...
    public static String encodeAccept(Accept accept) {
        var list = new ArrayList<String>();
        for (var mediaRange : accept) {
            list.add(mediaRange.encode());
        }
