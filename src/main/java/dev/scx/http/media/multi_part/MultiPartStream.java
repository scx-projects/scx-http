...
import dev.scx.http.headers.ScxHttpHeadersHelper;
...
public final class MultiPartStream implements MultiPart, Iterator<MultiPartPart>, AutoCloseable {
    ...
    public void consumeLastPart(MultiPartPart lastPart) throws MultiPartParseException {
    ...
    public ScxHttpHeadersWritable readHeaders(ScxHttpHeadersWritable headers) throws MultiPartParseException {
        ...
        return ScxHttpHeadersHelper.parseHeaders(headers, headersStr, true);// 使用严格模式解析
    }
    ...
    @Override
    public boolean hasNext() throws MultiPartParseException {
    ...
    @Override
    public MultiPartPart next() throws MultiPartParseException {
        if (!hasNext()) {
